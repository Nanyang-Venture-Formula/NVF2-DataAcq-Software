...
//
    if (HAL_GPIO_ReadPin(USR_KEY1_GPIO_Port, USR_KEY1_Pin))
    {
      ...
      {
        CDC_Transmit_HS((uint8_t *)"SD card not inserted\n", strlen("SD card not inserted\n"));
      }
      ...
      while (HAL_GPIO_ReadPin(USR_KEY1_GPIO_Port, USR_KEY1_Pin))
      {
        ;
      }
    }

    HAL_GPIO_WritePin(USR_LED_GPIO_Port, USR_LED_Pin, HAL_GPIO_ReadPin(SD1_DET_GPIO_Port, SD1_DET_Pin));
    HAL_Delay(500);
  }
  /* USER CODE END 3 */
}
